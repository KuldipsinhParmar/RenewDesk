...
try {
    if ($method === 'GET') {
        $stmt = $db->query("SELECT h.*, p.name as project_name FROM hosting h JOIN projects p ON h.project_id = p.id ORDER BY h.renewal_date ASC");
        $hosting = $stmt->fetchAll(PDO::FETCH_ASSOC);
        echo json_encode(["status" => "success", "data" => $hosting]);
    }
    elseif ($method === 'POST') {
    ...
    error_log("[hosting.php] " . $e->getMessage() . " in " . $e->getFile() . ":" . $e->getLine());
    http_response_code(500);
